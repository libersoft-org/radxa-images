# ROCK 5B Bookworm CLI Image via RSDK

Goal: build a ROCK 5B Bookworm CLI image from a clean Debian host using RSDK, optionally customize the rootfs, compress the release asset, and flash the image to an SD card.
...
ROCK 5B patch script then patches that checkout and creates the ROCK 5B build
helper inside RSDK. After both commands finish, the next DevContainer command
is only `rsdk devcon`.
...
After the scripted host setup and ROCK 5B RSDK patch have completed, enter the
already-started DevContainer. For a normal user the checkout is usually
`~/rsdk`; for a direct root run it is `/workspaces/rsdk`.
...
The ROCK 5B patch script modifies the host-side RSDK checkout. Since the
checkout is mounted as `/workspaces/rsdk`, the container sees the patched files
and the ROCK 5B build helper. The build helper installs required container
...
removes timed-out test containers named `rsdk-nettest-bridge-*` or
`rsdk-nettest-host-*`.
...
## 8. RSDK Patch: Enable CLI for ROCK 5B

The current RSDK `main` branch may not include `cli` for `rock-5b` in `products.json`. Add it locally.
...
packages. Otherwise `task-rock-5b` can pull in the `rtl8852bu-dkms` DKMS
package, which fails inside the mmdebstrap rootfs chroot.
...
## 10. Build the ROCK 5B Bookworm CLI Image
...
rm -rf out/rock-5b_bookworm_cli
...
/workspaces/rsdk/out/rock-5b_bookworm_cli/seed.tar.xz
/workspaces/rsdk/out/rock-5b_bookworm_cli/build-image
```
...
ls -lh out/rock-5b_bookworm_cli
ls -lh out/rock-5b_bookworm_cli/output.img
```
...
IMG="$HOME/rsdk/out/rock-5b_bookworm_cli/output.img"
...
RSDK build output:
  ...
  ~/rsdk/out/rock-5b_bookworm_cli/output.img.xz
  ~/rsdk/out/rock-5b_bookworm_cli/SHA256SUMS

SD card:
  contains the ROCK 5B Bookworm CLI image
  is unmounted
