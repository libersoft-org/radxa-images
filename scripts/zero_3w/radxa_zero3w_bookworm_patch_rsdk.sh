...
BUILD_HELPER_NAME="build-radxa-zero3-bookworm-cli.sh"
...
Patches an existing host-side RSDK checkout for a Radxa ZERO 3W / ZERO 3 Bookworm CLI build.
...
def walk(value):
    if isinstance(value, dict):
        if value.get("product") == "radxa-zero3":
...
if not matches:
    print('[ERROR] Could not find product object where "product" is "radxa-zero3".', file=sys.stderr)
    sys.exit(1)

if len(matches) > 1:
    print('[ERROR] Found multiple "radxa-zero3" product objects; refusing to guess.', file=sys.stderr)
    sys.exit(1)
...
if mode == "check":
    if not isinstance(editions, list):
        print('[ERROR] radxa-zero3 supported_edition is missing or is not a JSON array.', file=sys.stderr)
        sys.exit(1)
    if "cli" not in editions:
        print('[ERROR] radxa-zero3 supported_edition does not contain "cli".', file=sys.stderr)
        sys.exit(1)
    print('[OK] products.json contains "cli" in radxa-zero3 supported_edition.')
    sys.exit(0)
...
if not isinstance(editions, list):
    print('[ERROR] radxa-zero3 supported_edition exists but is not a JSON array.', file=sys.stderr)
    sys.exit(1)
...
if mode == "dry-run":
    if changed:
        print('[INFO] Would patch products.json so radxa-zero3 supports "cli".')
    else:
        print('[OK] products.json already has "cli" for radxa-zero3.')
    if format_changed:
        print('[INFO] Would write products.json as pretty JSON with indent=4.')
    print("[INFO] Resulting radxa-zero3 product object:")
...
if changed:
    print('[OK] Patched products.json so radxa-zero3 supports "cli".')
else:
    print('[OK] products.json already had "cli" for radxa-zero3.')
if format_changed:
...
OUT="out/radxa-zero3_bookworm_cli"
...
echo "Building Radxa ZERO 3W / ZERO 3 Bookworm CLI image..."
